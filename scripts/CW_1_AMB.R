...
# load libraries
library(rospca)
library(lattice) # may not need, check again
library(MASS)
...
df_all
df_1 = df_all$data[[1]] # get the raw data

# b.)
corr_df = cor(df_1) # check estimated correlation among the variables
sd_df = apply(df_1, 2 , sd)  # check estimated standard deviation values
corr_df
sd_df 
...
corr_df2 = cor(df_2) # check estimated correlation among the variables
sd_df2 = apply(df_2, 2 , sd)  # check estimated standard deviation values
corr_df2 # see some variables correlate, albeit not significant
sd_df2 
...
biplot(pr_1.out, scale = 0) # we see a high clustering of variables 7,8 and 9, and of variables 4,5, and 6
                            ...
                            # whereas the group 4,5 and 6 exhibits a vice versa phenomenon. Since, variables 1,2 and 3, are not as correlated
                            # as the other groups, the three have the same direction (signs) for both PC1 and PC2
### test when we did not standardize the variables
pr_1_uc.out = prcomp(df_1)
biplot(pr_1_uc.out, scale = 0)
...
apply(df_2, 2, var)
#### hence we should scale the data before doing PCA
...
biplot(pr_2.out, scale = 0) # this is so constrast with pr_1, as the red arrows, which are the first 
                            # two component loading vectors, are speard out in all directions 
                            # This is expected, as the variables are marginally correlated 
#### test when we did not standardize the variables
pr_2_uc.out = prcomp(df_2)
biplot(pr_2_uc.out, scale = 0)
# calculate variance explained by each principal component
pr_2.out$sdev
...
# principal we included, the more complexities in terms of interpretation
...
### In sum, we know that df_2 are not linearly correlated by construction, and hence the justification
...
# hence we standardize the variables first
df3.sc = scale(df3)
...
### create am emthy vector to store values
Stress = c()
k = c(1,2,3,4)

### create a for loop to extract stress values
for (i in k){
  to_store = isoMDS(df3.sc.dist, k = i)$stress
  Stress = append(Stress, to_store)
}

### plot the result
plot(k, Stress, type = "b", xlab = "Number of dimensions",
     sub = "Scree plot of stress by number of dimensions" ) # we can identify an elbow at k = 2
